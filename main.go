...
func (r Running) Calories() float64 {
	...
	calories := ((18*r.meanSpeed() + 1.79) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return calories
}
...
func (w Walking) Calories() float64 {
	...
	meanSpeedMS := w.meanSpeed() * KmHInMsec   // средняя скорость в метрах в секунду
	meanSpeedMSPow := math.Pow(meanSpeedMS, 2) // средняя скорость в метрах в секунду в степени 2
	calories := ((CaloriesWeightMultiplier*w.Weight + (meanSpeedMSPow/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return calories
}
